% ===========================================
% Local Sensitivity Analysis for J_pho (reparameterized)
% Params: [Ts_max, Tm_max, Ks_m, Km_m, Kp_m, lambda21, lambda31]
% ===========================================

clear; clc;

% Load phosphate/competition data
load('Palmier_DIC_Competition_Exp_Data.mat');  % e.g., M_with_S, etc.
...
TP_FIXED = 1;   % <-- set to the adopted literature value (units consistent with model)
...
LSC_MATRIX = zeros(6, n_params);  % LSCs for J_pho per condition
...
for j = 1
    ...
    % Compute baseline phosphate flux
    [~, ~, J_pho_base, ~] = compute_flux(y0, params0, Vm, Vims, TP_FIXED);
    ...
    for i = 1:n_params
        ...
        % Perturbed fluxes
        [~, ~, J_pho_plus,  ~] = compute_flux(y0, p_plus,  Vm, Vims, TP_FIXED);
        [~, ~, J_pho_minus, ~] = compute_flux(y0, p_minus, Vm, Vims, TP_FIXED);

        % Central finite difference derivative dJ/dp_i
        dJdpi = (J_pho_plus - J_pho_minus) / (2 * perturbation * params0(i));

        % Normalized local sensitivity coefficient for J_pho
...
mean_LSCs_pho = mean(LSC_MATRIX, 1);
save('LSCs_pho.mat','mean_LSCs_pho');
% ---------- Plot horizontal bar chart ----------
figure;
barh(mean_LSCs_pho, 'FaceColor', [0.2, 0.4, 0.6]);
...
title('Mean Local Sensitivity Coefficients for Phosphate Flux', 'FontSize', 18);
grid on;
